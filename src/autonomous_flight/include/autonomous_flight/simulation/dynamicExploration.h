...
namespace AutoFlight{
	class dynamicExploration : public flightBase{
	private:
		...
		ros::Subscriber ext_path_sub_;
		void externalPathCb(const nav_msgs::Path::ConstPtr& msg);
		ros::Subscriber request_op_sub_;
		void requestOpCb(const std_msgs::Empty::ConstPtr& msg);
		...
		ros::Subscriber requestPlanSub_;
		void requestPlanCB(const std_msgs::Empty::ConstPtr& msg);
		// Add subscriber declaration
		ros::Subscriber ugvInjectSub_;
		// Add callback declaration  
		void ugvInjectCB(const sensor_msgs::PointCloud2::ConstPtr& msg);
		// TF2 for transforming UGV point cloud
    	tf2_ros::Buffer tfBuffer_;
		tf2_ros::TransformListener tfListener_{tfBuffer_}; 
		ros::Subscriber ugvOdomSub_;
		Eigen::Vector3d ugvPosition_ = Eigen::Vector3d(0, 0, -1000);
		bool ugvOdomReceived_ = false;
		void ugvOdomCB(const nav_msgs::Odometry::ConstPtr& msg);
		ros::Subscriber ugv2DMapSub_;
		nav_msgs::OccupancyGrid::ConstPtr latestUgv2DMap_;
		void ugv2DMapCB(const nav_msgs::OccupancyGrid::ConstPtr& msg);
		// UGV path history for exclusion
		std::deque<Eigen::Vector3d> ugvPathHistory_;
		double ugvPathRecordDist_ = 0.0;  
	...
	public:
		...
		using flightBase::takeoff;
		using flightBase::moveToOrientation;
		using flightBase::stop;
		std::shared_ptr<globalPlanner::DEP> getPlanner() { return expPlanner_; }
		void triggerExplorationReplan() {
			this->explorationReplan_ = true;
			ROS_INFO("[AutoFlight] Exploration replan triggered.");
		}
