...
namespace globalPlanner{
	...
	bool DEP::makePlan() {
		...
		std::vector<int> best_idx_uav = this->selectOrienteeringPathGRASPPR(
    		order, id_of, start_idx, uav_end_idx, opp, this->uavCruiseSpeed_);
		...
		ugv_opp.T_MAX = std::max(15.0, min_time * 1.1);

		// 5. SOLVE OP
		std::vector<int> best_idx_ugv = this->selectOrienteeringPathGRASPPR(
    		ugv_order, ugv_id_of, ugv_start_idx, ugv_end_idx, ugv_opp, ugv_speed);
		...
		// // 6. Log Timing
		// ROS_INFO("========== UGV OP PATH TIMING ==========");
		// double total_ugv_time = 0.0;
		// for (size_t i = 0; i < best_idx_ugv.size() - 1; ++i) {
		// 	int u = best_idx_ugv[i];
		// 	int v = best_idx_ugv[i+1];
		// 	double segment_time = T_ugv[u][v];
		// 	double dist = (ugv_order[u]->pos - ugv_order[v]->pos).norm();
		// 	total_ugv_time += segment_time;
		// 	ROS_INFO("  Node %d -> Node %d | Dist: %.2fm | Time: %.2fs", 
		// 			i, i+1, dist, segment_time);
		// }
		// ROS_INFO("TOTAL UGV TIME: %.2fs", total_ugv_time);
		// ROS_INFO("========================================");
	...
	void DEP::buildRoadMap(){
		...
		for (std::shared_ptr<PRM::Node>& n : newNodes){
			...
			for (std::shared_ptr<PRM::Node>& nearestNeighborNode : knn){ 
				...
				if (distance2knn < this->maxConnectDist_ and rangeCondition){
					if (this->map_->isInflatedFreeLine(n->pos, nearestNeighborNode->pos)){
    ...
    double cruise_speed) // <--- ADDED PARAMETER HERE
	{
		std::vector<double> S;
	...
	int DEP::calculateUnknownUGV(const std::shared_ptr<PRM::Node>& n){
		// 1. Setup UGV Parameters
		// Use the node position (feet) + sensor height
		Eigen::Vector3d p = n->pos;
		p(2) += this->ugvSensorHeight_; 

		// USE UGV RANGE, NOT UAV dmax_
		double range_limit = this->ugvMaxRange_; 
		
		// TRICK: Use a wider FOV (e.g., 0.8 or 1.0 rad) to allow looking "over" low obstacles
		// If you use strict VLP-16 FOV (0.52), you might get 0 gain again.
		// Let's stick to the UAV's FOV since you liked that result, 
		// or tune this value:
		double effective_vertical_fov = this->verticalFOV_; // or 0.8; 

		double zRange = range_limit * tan(effective_vertical_fov / 2.0);
		int countTotalUnknown = 0;

		// 2. Iterate using the logic that worked for you
		for (double z = p(2) - zRange; z <= p(2) + zRange; z += this->map_->getRes()){
			for (double y = p(1) - range_limit; y <= p(1)+ range_limit; y += this->map_->getRes()){
				for (double x = p(0) - range_limit; x <= p(0) + range_limit; x += this->map_->getRes()){
					...
					// Global Bounds Check
					if (nodePoint(0) < this->globalRegionMin_(0) || nodePoint(0) > this->globalRegionMax_(0) ||
						nodePoint(1) < this->globalRegionMin_(1) || nodePoint(1) > this->globalRegionMax_(1) ||
						nodePoint(2) < this->globalRegionMin_(2) || nodePoint(2) > this->globalRegionMax_(2))
						continue;

					// 3. Status Check
					if (this->map_->isUnknown(nodePoint) && !this->map_->isInflatedOccupied(nodePoint)){
						
						// 4. Manual Distance/FOV Check (Inline implementation of sensorFOVCondition)
						Eigen::Vector3d direction = nodePoint - p;
						double distance = direction.norm();

						// Check UGV Range
						if (distance > range_limit) continue; 
						if (distance < 0.6) continue; // Self-filter

						// Check Vertical Angle (To match zRange logic)
						// (Optional: If you want to rely purely on zRange loop limits, skip this)
						
						// 5. Raycast (The check that works for you)
						// The UAV function uses: isInflatedOccupiedLine -> false
