...
namespace globalPlanner{
	...
	private:
		...
		bool reached_80_percent_;
		bool reached_95_percent_;
		ros::Time time_80_percent_;
		ros::Time time_95_percent_;
		...
		int planning_iterations_;
		ros::Publisher completion_pub_;
	...
	public:
		...
		void startExplorationTimer() {
			...
			ROS_INFO("[DEP] Exploration timer started at %.2f", exploration_start_time_.toSec());
		}

		bool reached80Percent() const { return reached_80_percent_; }
		bool reached95Percent() const { return reached_95_percent_; }
		double getTimeTo80Percent() const { 
			return reached_80_percent_ ? (time_80_percent_ - exploration_start_time_).toSec() : -1.0;
		}
		double getTimeTo95Percent() const { 
			return reached_95_percent_ ? (time_95_percent_ - exploration_start_time_).toSec() : -1.0;
		}
		...
		std::shared_ptr<PRM::Node> randomConfigUGV(const Eigen::Vector3d& minRegion, 
                                                 const Eigen::Vector3d& maxRegion);

		void buildRoadMap();
		void buildUGVRoadMap();
		...
		bool isPosValidUGV(const Eigen::Vector3d& p);
		...
		std::shared_ptr<PRM::Node> sampleFrontierPointUGV(const std::vector<double>& sampleWeights);
		std::shared_ptr<PRM::Node> extendNode(const std::shared_ptr<PRM::Node>& n, const std::shared_ptr<PRM::Node>& target);
		std::shared_ptr<PRM::Node> extendNodeUGV(const std::shared_ptr<PRM::Node>& n, 
