...
namespace globalPlanner{
	...
	void DEP::initParam(){
		...
		if (!this->nh_.getParam(this->ns_ + "/prm_ground_fraction", this->gamma_))
    		this->gamma_ = 0.4; // Default value if param not set
	}
	...
	bool DEP::makePlan(){
		...
		// this->printPRMNodesInformationGain(); // ******Update *****
  ...
  //   	params.minConvexity = 0.1;;
	...
	void DEP::buildRoadMap(){
		...
		while (ros::ok() && not saturate) {
			if (regionSaturate) {
				int countFailureGlobal = 0;
				int totalGlobalSamples = this->globalSampleThresh_;
				std::cout << "totalGlobalSamples: " << totalGlobalSamples;
				int numGroundNodes = int(this->gamma_ * totalGlobalSamples);
				std::cout << "numGroundNodes: " << numGroundNodes;
				int numAirNodes = totalGlobalSamples - numGroundNodes;
				int countSampledGround = 0;
				int countSampledAir = 0;

				// ---- Ground nodes: z = 0 ----
				while (ros::ok() && countSampledGround < numGroundNodes) {
					if (countFailureGlobal > totalGlobalSamples) {
						saturate = true; break;
					}
					Eigen::Vector3d groundMin = this->globalRegionMin_;
					Eigen::Vector3d groundMax = this->globalRegionMax_;
					groundMin(2) = groundMax(2) = 0.0;

					n = this->randomConfigBBox(groundMin, groundMax);
				...
				//---- Airspace nodes: z > 0 ----
				while (ros::ok() && countSampledAir < numAirNodes) {
					if (countFailureGlobal > totalGlobalSamples) {
						saturate = true; break;
					}
					Eigen::Vector3d airMin = this->globalRegionMin_;
					Eigen::Vector3d airMax = this->globalRegionMax_;
					airMin(2) = std::max(airMin(2), 0.01); // Avoid z=0 if desired

					n = this->randomConfigBBox(airMin, airMax);
					std::cout << "Air Nodes: ";
					double distToNN;
					if (this->roadmap_->getSize() != 0) {
						std::shared_ptr<PRM::Node> nn = this->roadmap_->nearestNeighbor(n);
				...
				std::cout << "countSampledAir: " <<countSampledAir;
			}
	...
	// 	for (auto& node : this->prmNodeVec_) {
	...
	void DEP::printPRMNodesInformationGain() {
		...
		for (const auto& node : this->prmNodeVec_) {
			std::string nodeType = (std::abs(node->pos(2)) < 1e-3) ? "GROUND" : "AIR";
