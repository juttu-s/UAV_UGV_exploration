...
namespace mapManager{
	...
	public:
		...
		void registerPub();
		// void inject2DFreeSpace(const nav_msgs::OccupancyGrid::ConstPtr& grid, double zMin = 0.0, double zMax = 0.5);
		void inject2DFreeSpace(const nav_msgs::OccupancyGrid::ConstPtr& grid, double floorHeight = 0.0);
		// void injectPointCloud(const sensor_msgs::PointCloud2::ConstPtr& cloud);
		void injectPointCloud(const sensor_msgs::PointCloud2::ConstPtr& cloud, 
                      const Eigen::Vector3d& excludeCenter = Eigen::Vector3d(0,0,-1000), 
                      double excludeRadius = 0.0);
		void injectPointCloudWithExclusions(const sensor_msgs::PointCloud2::ConstPtr& cloud,
                                     const std::vector<std::pair<Eigen::Vector3d, double>>& exclusions);
		void getExplorationStatistics(
									int& explored_voxels,
									int& free_voxels, 
									int& occupied_voxels) ;
		double getExploredVolume();
		int getExploredVoxelCount();
		void clearFootprints(const std::vector<Eigen::Vector3d>& positions, double radius, double zMin = -0.5, double zMax = 2.0);
